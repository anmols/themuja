...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    ...
    foreach($data as $key=>$val)
    {
        if(is_array($val))
        {
            $data_to_db[$key."_json"] = json_encode($val);
        }
        else {
            $data_to_db[$key] = $val;
        }
    ...
    $data_to_db['updatedBy'] = $_SESSION['user_id'];
    $data_to_db['updatedOn'] = date('Y-m-d H:i:s');

    $db = getDbInstance();
    $db->where('id', $customer_id);
    $stat = $db->update('customer', $data_to_db);
...
if ($edit)
{
    ...
    foreach($customer as $key=>$val)
    {
        if(strstr($key,'_json'))
        {
            $customer[str_replace("_json", "", $key)] = json_decode($val,TRUE);
        }
    }
